...
for pat = 1 %: length(data(:, 1))       % Patients = 4
    for cur_trial = 1 %: trial(1, pat)  % Walking trials = [8, 5, 8, 6]
        ...
        xlim([0 1.2])
        ylim([-120 120])
        plot(data{pat, cur_trial}(1,1), data{pat, cur_trial}(1,4), '+r')
        plot(data{pat, cur_trial}(1,1), data{pat, cur_trial}(1,5), 'or')
        % Plot Reaction Forces at Ankle joint at x, y, z.
        plot(data{pat, cur_trial}(1,1),...
            force_LCS{pat, cur_trial}(1).ankle(1),...
            '.g', 'MarkerSize', 10)
        plot(data{pat, cur_trial}(1,1),...
...
for pat = 1 %: length(data(:, 1))       % Patients = 4
    for cur_trial = 1 %: trial(1, pat)  % Walking trials = [8, 5, 8, 6]
        figure(15)
        subplot(2,2,[1,2])
        title(['Patient ' num2str(pat) ' Wa;ling - Reaction Moments'])
        hold on
        xlabel('time (s)')
