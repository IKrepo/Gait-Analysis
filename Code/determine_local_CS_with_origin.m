...
%         ez = ez/norm(ez);
    ...
    case 3 % thigh P3,5,7
        ez=(p2-p3)/norm(p2-p3); % !! different from shank
        ex_tmp = (p1-p2)/norm(p1-p2); % Suggested one. Based on the
        ey = cross(ez,ex_tmp);        % description it doesn't seem valid.
        ey=ey/norm(ey);               % Description and paper agree.
        ex=cross(ey,ez);
        ex=ex/norm(ex);
